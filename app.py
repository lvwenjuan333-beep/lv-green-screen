...
def loop_audio(audio_clip, duration):
    if audio_clip is None or duration <= 0:
        return None
    full_dur = audio_clip.duration
    n_full = int(duration // full_dur)
    remainder = duration % full_dur
    clips = []
    for _ in range(n_full):
        clips.append(audio_clip)
    if remainder > 0:
        clips.append(audio_clip.set_duration(remainder))
    if not clips:
        return None
    return clips[0] if len(clips) == 1 else concatenate_audioclips(clips)
            ...
            with col1:
                ...
                if st.button("🔥 开始全片大卡高规格合成", type="primary", use_container_width=True):
                    with st.spinner("影视级抗噪自适应算法深度处理中，请稍候..."):
                        ...
                        green_duration = int(cap_green.get(cv2.CAP_PROP_FRAME_COUNT)) / fps if fps > 0 else 0
                        ...
                        # 等待文件写入完成
                        while not os.path.exists(silent_path):
                            pass
                        
                        # 音频处理
                        green_audio = safe_extract_audio(st.session_state.green_cache_path)
                        game_audio = safe_extract_audio(st.session_state.game_cache_path)
                        clips = []
                        if green_audio: clips.append(set_volume(green_audio, 1.0))  # 音量固定为1，如需调节可加侧边栏
                        if game_audio:
                            looped = loop_audio(game_audio, green_duration)
                            if looped: clips.append(set_volume(looped, 1.0))
                        mixed_audio = None
                        if len(clips) == 1:
                            mixed_audio = clips[0]
                        elif len(clips) >= 2:
                            mixed_audio = CompositeAudioClip(clips)
                        else:
                            st.warning("两个素材都无音轨，导出视频将为无声。")
                        ...
                        if mixed_audio is not None:
                            ...
                            final_clip = final_clip.with_duration(min(final_clip.duration, mixed_audio.duration))
                        
                        final_clip.write_videofile(
                            output_final_path,
                            codec='libx264',
                            audio_codec='aac' if mixed_audio is not None else None,
                            logger=None
                        )
                        final_clip.close()
